...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = ((df[(df['hours-per-week'] == min_work_hours) & (df['salary'] == '>50K')].shape[0]) / (df[df['hours-per-week'] == min_work_hours].shape[0])) * 100
    ...
    highest_earning_country_percentage = round(((df[df['salary'] == '>50K']['native-country'].value_counts() / df['native-country'].value_counts()) * 100).sort_values(ascending=False),1).max()
